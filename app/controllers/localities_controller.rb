class LocalitiesController < ApplicationController
  ...
  def index
    ...
    @localities = @localities.order(:province_name).page(params[:page] || 1)
    @localities = @localities.where(["name LIKE :filter", :filter => "%" + params[:filter] + "%"]) unless params[:filter].blank?
  ...
  private
    ...
    def locality_params
      params.require(:locality).permit(:name, :province_id)
    end
end
